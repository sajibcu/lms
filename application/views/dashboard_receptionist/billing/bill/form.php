    ...
    <div class="col-sm-12 col-md-12">
        <div class="panel panel-bd billing-panel ">
            ...
            <div class="panel-heading no-print row">
                ...
                <h2 class="col-xs-10 text-left text-success"><?php echo "Add Discharge Bill" ?></h2>
            </div> 
 
            <div class="panel-body">
            <?php echo form_open('dashboard_receptionist/billing/bill/form', array('class'=>'billig-form')) ?>
                <div class="row">
                    <div class="col-sm-9">
                        <div class="row">
                                    ...
                                    <tr>
                                        <td>
                                            <div class="input-group">
                                              ...
                                              <input type="text" id="discountPercent" required="" autocomplete="off" class="form-control tax-discount-calc" value="0.00">
                                            </div>
                                        </td>
...
$(document).ready(function(){
    ...
    aid.on('blur',function(){
        ...
        $.ajax({
            url: '<?php echo base_url('dashboard_receptionist/billing/bill/getInformation') ?>',
            method: 'post',
            dataType: 'json',
            data: {
            ...
            {  
                ...
                if (data.status==true)
                {
                    ...
                    //alert(1+'#'+data.result.picture+'#'+data.result.sex);
                    if((data.result.picture=='' ||data.result.picture==null) && data.result.sex=="female"){
                        picture.attr('src','<?= base_url() ?>assets/images/patient/female.png');
                    }else if((data.result.picture=='' || data.result.picture==null) && data.result.sex=="Male"){

                        picture.attr('src','<?= base_url() ?>assets/images/patient/male.png');
                    }else{
                    ...
                    discharge_date.val('<?=date("Y-m-d")?>');
                    var totaldays = countDay('<?=date("Y-m-d")?>',data.result.admission_date);
                    ...
                    //alert(data.result.discount);
                    if(data.result.discount=='' || data.result.discount==null ){
                        discount.val(0.0);
                    }else{
    ...
    var options = {
      ...
      source: [
        ...
        foreach ($service_list as $service):
            if($comma_flag == true) echo ", ";
            echo "{label:'".addslashes($service->name)."', service_id:'$service->id', quantity:'$service->quantity', amount:'$service->amount'}";
            $comma_flag = true; 
        endforeach;
        ?>
        ],
...
function countDay(firstDate,secondDate){
