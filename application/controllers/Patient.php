...
class Patient extends CI_Controller {
	...
	public function __construct()
	{
		...
		if ($this->session->userdata('isLogIn') == false
			|| ($this->session->userdata('user_role') != 1 && $this->session->userdata('user_role') != 7) ) 
			redirect('login');
	}
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('age', display('age'),'max_length[5]');
		...
		if ($this->input->post('id') == null) { //create a patient
			$data['patient'] = (object)$postData = [
				'id'   		  	 => $this->input->post('id'),
				'patient_id'  	 => date('ymd').random_string('numeric', 6),
				'firstname'   	 => $this->input->post('firstname'),
				'lastname' 	   	 => $this->input->post('lastname'),
				'fatherOrSpouse' => $this->input->post('fatherOrSpouse'),
	...
	public function search_by_name(){
        ...
        if($name){
            $data = $this->patient_model->getItemsListBySearch($name, 20);
        }
    ...
    */
    ...
    public function getPaientInfo()
    {
    	$aid = $this->input->post('aid');
    	$data = array();
    	if($aid !=""){
    		$data = $this->patient_model->patientInfoByAid($aid);
    	}
    	echo json_encode($data);
    }
