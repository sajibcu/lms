...
class advance extends CI_Controller {
	...
	public function __construct()
	{
		...
		if ($this->session->userdata('isLogIn') == false 
			|| ($this->session->userdata('user_role') != 1 && $this->session->userdata('user_role') != 3)
		) 
		redirect('login'); 

		// if ($this->session->userdata('isLogIn') == false || ($this->session->userdata('user_role') != 1 && $this->session->userdata('user_role') != 3) 
		// 	redirect('login');
	}
	...
	public function form($id = null)
	{
		...
		$this->form_validation->set_rules('cash_card_or_cheque', display('payment_method'),'required|max_length[10]');
    ...
    public function view($id = null,$patient_id = null) 
	{
		...
		$data['content'] = $this->load->view('billing/advance/view',$data,true);
