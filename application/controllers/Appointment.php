...
class Appointment extends CI_Controller {
    ...
    public function create()
    {
        ...
        $data['appointment'] = (object)$postData = [
            ...
            'appointment_type'     => $this->input->post('appointment_type',true) ? $this->input->post('appointment_type',true) : 'First',
        ...
        // additional JS
        $data['additional_js'] = array('lab_report.js');
    ...
    */
    ...
    public function serial_by_date()
    {
        $patient_id = $this->input->post('patient_id');
        $doctor_id  = $this->input->post('doctor_id');
        $doctor = $this->doctor_model->read_by_id($doctor_id);
        ...
        if (!empty($doctor_id) && !empty($patient_id) && !empty($day)) {
            ...
            if ($query->num_rows() > 0) {
                ...
                $data['fee1']     = $doctor ? $doctor->fee1: 0;
                $data['fee2']     = $doctor ? $doctor->fee2: 0;
                $data['message']     = $serial;
                $data['status']      = true;
                /*--------- ------------------------------- */  
            } else {
