...
class advance extends CI_Controller {
	...
	public function __construct()
	{
		...
		if ($this->session->userdata('isLogIn') == false || $this->session->userdata('user_role') != 7) 
			redirect('login');
	}
	...
	{ 
        ...
        #pagination ends
        #    
		$data['content'] = $this->load->view('dashboard_receptionist/billing/advance/list',$data,true);
		$this->load->view('dashboard_receptionist/main_wrapper',$data);
	}
	public function view($id = null,$patient_id = null) 
	{
		...
		$data['advance_data'] = $this->advance_model->read_by_id($id);
		$data['content'] = $this->load->view('dashboard_receptionist/billing/advance/view',$data,true);
	...
	public function form($id = null)
	{
		...
		$this->form_validation->set_rules('cash_card_or_cheque', display('cash_card_or_cheque'),'required|max_length[10]');
		...
		{
			$receipt_no = date('ymd').random_string('numeric', 4);
		}else{
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($id)) 
			{
				$insert_id = $this->advance_model->create($postData);
				if ($insert_id) {
					$this->session->set_flashdata('message', display('save_successfully'));
					redirect('dashboard_receptionist/billing/advance/view/'.$insert_id.'/'.$data['advance']->patient_id);
				} 
				else 
				{
					$this->session->set_flashdata('exception', display('please_try_again'));
				} 
				redirect('dashboard_receptionist/billing/advance/form/');
			}
			else 
			{
				...
				redirect('dashboard_receptionist/billing/advance/form/'.$id);
			}
			...
			$data['content'] = $this->load->view('dashboard_receptionist/billing/advance/form',$data,true);
			$this->load->view('dashboard_receptionist/main_wrapper',$data);
		} 
	}
    ...
    public function delete($id = null) 
    {
		...
		redirect('dashboard_receptionist/billing/advance/index');
    }
