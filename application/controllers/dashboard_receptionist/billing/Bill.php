...
class Bill extends CI_Controller {
	...
	{ 
        ...
        #pagination ends
        #    
		$data['content'] = $this->load->view('dashboard_receptionist/billing/bill/list',$data,true);
	...
	public function form()
	{
			    ...
			    array(
			        ...
			        function($value)
			        {
						$rows = $this->db->select("admission_id")
			             	...
			             	->get()
			             	->num_rows();
			             $dischargeData = $this->db->select("*")
			             	->from("bill")
			             	->where("admission_id", $value)
			             	->get()
			             	->num_rows();
			            if($dischargeData>0){
			            	$this->form_validation->set_message('admission_callable', 'This patient already discharged!');
		...
		if ($this->form_validation->run()) {
			...
			if ($this->bill_model->create($postData)) {
				...
				$this->db->query("update bill_admission set discharge_date=CURDATE() where admission_id='".$admission_id."'");
				#-------------------------------#
				...
				{
					redirect("dashboard_receptionist/billing/bill/view/".$postData['bill_id']);
				}
			...
			redirect('dashboard_accountant/billing/bill/form/');
		} else {  
			$data['service_list'] = $this->service_model->readList();
			$data['content'] = $this->load->view('dashboard_receptionist/billing/bill/form',$data,true);
			$this->load->view('dashboard_receptionist/main_wrapper',$data);
		} 
	}
	...
	public function view($bill_id = null)
	{
		...
		$data['content'] = $this->load->view('dashboard_receptionist/billing/bill/view',$data,true);
		$this->load->view('dashboard_receptionist/main_wrapper',$data);
	}
	...
	public function edit($bill_id = null)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			redirect('dashboard_receptionist/billing/bill/edit/'.$postData['bill_id']);
		} else {   
			...
			$data['advance'] = $this->bill_model->advance_payment($bill_id); 
			$data['service_list'] = $this->service_model->readList(); 
			$data['content'] = $this->load->view('dashboard_receptionist/billing/bill/edit',$data,true);
			$this->load->view('dashboard_receptionist/main_wrapper',$data);
		} 
	}
    ...
    public function delete($id = null) 
    {
		...
		redirect('dashboard_receptionist/billing/bill/index');
    }
