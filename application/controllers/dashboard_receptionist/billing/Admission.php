...
class Admission extends CI_Controller {
	...
	{ 
        ...
        #pagination ends
        #    
		$data['content'] = $this->load->view('dashboard_receptionist/billing/admission/list',$data,true);
	...
	{ 
		...
		$this->form_validation->set_rules('guardian_name', display('guardian_name'),'required|max_length[255]');
		$this->form_validation->set_rules('guardian_relation', display('guardian_relation'),'required|max_length[255]');
		$this->form_validation->set_rules('guardian_contact', display('guardian_contact'),'required|max_length[255]');
		$this->form_validation->set_rules('guardian_address', display('guardian_address'),'required|max_length[255]');
		...
		$data['admission'] = (object)$postData = array(
			'admission_id'    	=> date('ymd').random_string('numeric', 5), 
	...
	{ 
		...
		if ($this->form_validation->run() === true) {
			...
			redirect('dashboard_receptionist/billing/admission/form/'.$admission_id);
			...
			$data['package_list'] = $this->admission_model->package_list();
			$data['insurance_list'] = $this->admission_model->insurance_list();
			$data['content'] = $this->load->view('dashboard_receptionist/billing/admission/edit',$data,true);
			$this->load->view('dashboard_receptionist/main_wrapper',$data);
		} 
	}
    ...
    public function delete($id = null) 
    {
		...
		redirect('dashboard_receptionist/billing/admission/index');
    }
